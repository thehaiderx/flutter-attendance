...
class _AttendanceScreenState extends State<AttendanceScreen> with SingleTickerProviderStateMixin {
  ...
  Map _processAttendanceData(Map att) {
    ...
    att['display_shift'] = "${att['shift_start_for_json'] ?? '--'} - ${att['shift_end_for_json'] ?? '--'}";
    att['display_late'] = att['is_late'] == true ? (att['late_time'] ?? "00:00:00") : "00:00:00";
  ...
  void _initTimer() {
    ...
    if (attendance?['check_in_time'] != null && attendance?['check_out_time'] == null) {
      _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
        if (mounted) {
          setState(() {
            attendance!['worked_seconds_computed'] = (attendance!['worked_seconds_computed'] ?? 0) + 1;
          });
        }
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Positioned(top: -50, right: -50, child: _buildOrb(Colors.blueAccent.withOpacity(0.08), 200)),
          SafeArea(
    ...
    return Row(
      ...
      children: [
        Column(
          ...
          children: [
            Text(formattedDate.toUpperCase(), style: TextStyle(color: Colors.blueAccent.withOpacity(0.8), fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
            const SizedBox(height: 4),
            Text(userData?['name'] ?? "Employee", style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900)),
          ],
        ),
        Row(
          children: [
            IconButton(onPressed: fetchAttendance, icon: const Icon(Icons.refresh_rounded, color: Colors.white24)),
            GestureDetector(
              onTap: _showLogoutDialog,
              child: Container(
                padding: const EdgeInsets.all(2),
                decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.blueAccent.withOpacity(0.3))),
                child: CircleAvatar(
                  radius: 20,
  ...
  Widget _buildMainTimer(bool active, bool finished) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: active ? Colors.blueAccent.withOpacity(0.2) : Colors.white.withOpacity(0.05)),
      ),
      child: Column(
        children: [
          Text(active ? "SESSION ACTIVE" : (finished ? "SHIFT COMPLETED" : "READY TO WORK"),
              style: TextStyle(color: active ? Colors.blueAccent : Colors.white38, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 2)),
          const SizedBox(height: 15),
          Text(
            formatHMS(attendance?['worked_seconds_computed'] ?? 0),
            style: const TextStyle(color: Colors.white, fontSize: 52, fontWeight: FontWeight.w200, fontFamily: 'monospace'),
          ),
          const SizedBox(height: 20),
          Row(
          ...
          child: isLoading 
            ...
            : Text(
                ...
                style: TextStyle(
                  ...
                  fontWeight: FontWeight.bold, letterSpacing: 1.5, fontSize: 14
                ),
              ),
        ),
      ),
  ...
  Future<void> _handleAttendanceAction() async {
    setState(() => isLoading = true);
    String endPoint = attendance?['check_in_time'] == null ? '/attendance/check-in' : '/attendance/check-out';
    try {
  ...
  void _showLogoutDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Are you sure you want to logout from Atom Soft?", style: TextStyle(color: Colors.white60)),
        actions: [
